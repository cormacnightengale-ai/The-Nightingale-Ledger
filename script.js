...
function addHabit(name, assignedTo) {
...
document.getElementById("habitForm").addEventListener("submit", (e) => {
  ...
  const checkboxes = document.querySelectorAll("input[name='assignTo']:checked");
  const assignedTo = Array.from(checkboxes).map(cb => cb.value);
  addHabit(name, assignedTo);
});
...
function renderHabits() {
  ...
  db.collection("habits").where("assignedTo", "array-contains", user.uid)
